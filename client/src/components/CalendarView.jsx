...
// Fixes events being invisible/clipped in shorter (non-fullscreen) viewports.
// react-big-calendar's month rows need explicit min-height + visible overflow,
// otherwise event pills get cut off when the container is shorter than the
// rows "want" to be (browser chrome reduces available height before F11).
const CALENDAR_HEIGHT_FIX = `
  .rbc-month-view {
    min-height: 600px;
  }
  .rbc-month-row {
...
const CalendarView = ({ onSelectEvent }) => {
  ...
  return (
    ...
    >
      <style>{CALENDAR_HEIGHT_FIX}</style>
      <Calendar
        localizer={localizer}
        events={events}
        startAccessor="start"
