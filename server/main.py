...
def _enrich_profile_with_source_metadata(profile: dict, pdf_path: str) -> dict:
    """
    Opens the PDF, searches for each field's quote or value, and adds:
    - page
    ...
    if pdf_path and os.path.exists(pdf_path):
        try:
            ...
            print(f"Error opening PDF for metadata enrichment: {e}")

    for field_name, field_data in profile.items():
        if field_name == "_deep_analysis" or field_name == "line_items":
            continue
        ...
        bbox = None
        confidence = 0.95
        source_text = quote or value or ""

        if history and len(history) > 0:
            confidence = 1.0
        if not value or str(value).lower() in ("not_found", "not found", "", "null", "none"):
            confidence = 0.0

        found = False
        if doc and value and confidence > 0:
            queries = []
            if quote and quote.strip() and quote != "Quote missing":
                queries.append(quote.strip())
            if isinstance(value, str) and value.strip() and value != "not_found":
        ...
        field_data["boundingBox"] = bbox
        field_data["confidence"] = confidence
        field_data["sourceText"] = source_text

    # Enrich line items
    if "line_items" in profile and doc:
        line_items_data = profile["line_items"]
        if isinstance(line_items_data, dict) and "value" in line_items_data:
            items_list = line_items_data["value"]
...
def _build_profile_from_record(record: PurchaseOrderRecord) -> dict:
    ...
    # Enrich with coordinate bounding boxes using PyMuPDF
    profile = _enrich_profile_with_source_metadata(profile, record.pdf_file_path)
...
async def process_contract_profile(file: UploadFile = File(...), db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {
            "filename": file.filename,
            "db_id":    db_record.id,
            "extraction_result": {"status": "success", "profile": _build_profile_from_record(db_record)}
        }
